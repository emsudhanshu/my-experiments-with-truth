import os
import sys
import shutil # Used for deleting the old vector store directory
...
# --- CONFIGURATION ---
FILE_PATH = "./Kakkar,Sudhanshu.pdf" # Ensure your PDF file is named exactly this
...
def setup_conversational_rag_chain():
    ...
    # *** CRITICAL FIX: AGGRESSIVE RETRIEVAL PRECISION (k=1) ***
    # This forces the retriever to only grab the single, highest-scoring chunk,
    # solving the "name not found" error for simple facts.
    ...
    # --- FINE TUNED SYSTEM PROMPT FOR CONCISENESS AND ACCURACY ---
    document_chain_prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "For simple, factual questions (like 'What is the name?' or 'When did he start?'), **provide the answer directly and concisely without any introductory phrases or extra sentences.** "
                "Example: 'Sudhanshu Kakkar' or 'January 2020'."
                "\n\nCONTEXT:\n{context}",
            ),
            ("placeholder", "{chat_history}"),
            ("human", "{input}"),
...
def run_qa_bot(rag_chain):
    ...
    global chat_history
    print("\n" + "="*50)
    print("      🚀 Conversational Resume Q&A Bot is Ready 🚀")
    print("="*50)
    print("Ask questions about the resume. Followup questions are now supported! Type 'exit' or 'quit' to stop.")
...
if __name__ == "__main__":
    ...
    # Automatically delete the old vector store to guarantee a fresh index with k=1
